...
def main(preprocessed_data_file_name,preprocessing_technic,caboosted):
    ...
    optimized_max_depth =   {'thesaurusBT':10,
                    'pivot':100,
                    'minhash':100}
    optimized_n_estimators = 1000
    ...
    clf = RandomForestClassifier(max_depth=optimized_max_depth[preprocessing_technic], random_state=42,n_estimators=optimized_n_estimators)
    multi_target_forest = MultiOutputClassifier(clf)
    y_pred = multi_target_forest.fit(X_train[:,1:], y_train).predict(X_test[:,1:])
    y_pred = clf.fit(X_train[:,1:], y_train).predict(X_test[:,1:])
    ...
    np.savetxt(repo_path+"outputs/processing/"+output_file_name+"_rf_pred.csv", y_pred, delimiter=";")
    np.savetxt(repo_path+"outputs/processing/"+output_file_name+"_rf_test.csv", y_test, delimiter=";")
    ...
    print("BR RF multi-labelling time of execution : ",end_time_multi_labelling - start_time_multi_labelling)
